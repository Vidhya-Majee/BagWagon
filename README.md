...
🌐 **Live Demo:** [bagwagon.onrender.com](https://bagwagon.onrender.com)
...
```bash
git clone https://github.com/Vidhya-Majee/BagWagon.git
cd BagWagon
```
...
```env
PORT=3000
MONGODB_URI=mongodb://localhost:36116/bagwagon
JWT_KEY=myjwtsecretkey123
EXPRESS_SESSION_SECRET=mysessionsecretkey456
CORS_ORIGIN=*
```

> ⚠️ Replace these with your own secret values. Never share your real `.env` file publicly.
...
BagWagon is deployed using **Render** (hosting) + **MongoDB Atlas** (cloud database) — both completely free.

| Service | Purpose | Link |
|---|---|---|
| Render | Hosts the Node.js app | [render.com](https://render.com) |
...
| `JWT_KEY` | Any random secret string |
| `EXPRESS_SESSION_SECRET` | Any random secret string |
| `CORS_ORIGIN` | `*` |
| `NODE_ENV` | `production` |

> **Note:** Render's free tier sleeps after 15 minutes of inactivity. The first request may take ~30 seconds to wake up.
